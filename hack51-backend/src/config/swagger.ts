...
\`\`\``,
  ...
  paths: {
    ...
    "/candidate/uploads/sign": { post: { tags: ["Candidate – Submissions"], summary: "Get a signed URL to upload an artifact file", description: "For artifact_type 'upload' or 'both'. Returns a signed Supabase Storage URL. Upload the raw file directly to `upload_url` (PUT), then submit the challenge with the returned `public_url` inside `artifact_urls`. The file never passes through the API, so there's no payload size limit on the API side (bucket cap is 50MB). Requires the `artifacts` storage bucket (migration 003).", security: [{ bearerAuth: [] }], requestBody: { required: true, content: { "application/json": { example: { filename: "my-design.pdf", content_type: "application/pdf" } } } }, responses: { 200: { description: "Returns { upload_url, token, path, public_url, bucket }" }, 400: { description: "FILENAME_REQUIRED | CONTENT_TYPE_REQUIRED | UNSUPPORTED_FILE_TYPE" } } } },
    "/candidate/challenges/{id}/submit": { post: { tags: ["Candidate – Submissions"], summary: "Submit to challenge", description: "For file uploads, first call POST /candidate/uploads/sign, upload the file, then pass the returned public_url here in artifact_urls with artifact_type 'upload'.", security: [{ bearerAuth: [] }], requestBody: { required: true, content: { "application/json": { example: { artifact_urls: ["https://github.com/user/repo"], artifact_type: "link", submission_statement: "All my own work.", integrity_declared: true } } } }, responses: { 201: { description: "Submission received" }, 409: { description: "Already submitted" }, 403: { description: "Challenge closed or deadline passed" } } } },
