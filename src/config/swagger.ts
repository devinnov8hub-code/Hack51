...
\`\`\``,
  ...
  paths: {
    ...
    "/admin/review/shortlists/{requestId}/confirm":        { post: { tags: ["Admin – Review"], summary: "Confirm top-N shortlist (admin_lead+)", description: "Replace the `candidate_id` and `submission_id` values with real UUIDs from `GET /admin/review/shortlists/{requestId}/candidates` (use `users.id` for candidate_id and `id` for submission_id from that response). The example UUIDs below are illustrative — sending them as-is will return 404 because they do not exist in the database. Last call wins (idempotent) — you can re-confirm a different selection until you call `/deliver`.", security: [{ bearerAuth: [] }], requestBody: { required: true, content: { "application/json": { example: { selections: [{ candidate_id: "11111111-1111-1111-1111-111111111111", submission_id: "22222222-2222-2222-2222-222222222222", rank: 1 }, { candidate_id: "33333333-3333-3333-3333-333333333333", submission_id: "44444444-4444-4444-4444-444444444444", rank: 2 }, { candidate_id: "55555555-5555-5555-5555-555555555555", submission_id: "66666666-6666-6666-6666-666666666666", rank: 3 }] } } } }, responses: { 200: { description: "Shortlist confirmed" }, 400: { description: "EMPTY_SHORTLIST — selections array cannot be empty | SUBMISSION_REQUEST_MISMATCH — submission belongs to a different request | CANDIDATE_SUBMISSION_MISMATCH — candidate_id does not match submission's candidate | SUBMISSION_NOT_SCORED — only scored submissions can be shortlisted" }, 404: { description: "REQUEST_NOT_FOUND or SUBMISSION_NOT_FOUND — one of the IDs does not exist" }, 422: { description: "VALIDATION_ERROR — candidate_id / submission_id must be valid UUIDs" } } } },
